...
use log::*;
use crate::{ApiBase, FramebufferStorage, RenderPassSafe, DescriptorPoolSafe, DeviceSafe, SwapchainSafe, GraphicBase, RenderCamera, ServerTexture, Pools, TextureServer};


struct TextureTransformPipeline {
...
impl TextureTransformPipeline {
    ...
    fn init_base_pipeline(
        ...
        renderpass: &RenderPassSafe) -> Result<(vk::Pipeline, vk::PipelineLayout, Vec<vk::DescriptorSetLayout>), Box<dyn std::error::Error>> {
            ...
            let desc_set_normal = TextureTransformPipeline::get_img_desc_set(logical_device.clone());
            let desc_set_met_roug = TextureTransformPipeline::get_img_desc_set(logical_device.clone());
    ...
    pub fn new(
        ...
        let renderpass = TextureTransformPipeline::init_renderpass(&graphic_base).unwrap();
