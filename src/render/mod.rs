...
use crate::{mesh::{GpuMesh, Vertex}, rpu::RPU, game_object::{Pos, DirectLight}};
use vulkano::image::*;
...
pub struct DirLightShadowRender {
    pub rpu : RPU,
    pub pipeline : Arc<GraphicsPipeline>,
    pub render_pass : Arc<RenderPass>,
    pub viewport : Viewport,
    ...
    pub fn draw(&mut self, world : &World, camera : &Camera) {
        ...
        match dir_light.textures.clone() {
            Some(texs) => {
                ...
                let uniform_buffer = camera.get_uniform_buffer(self.rpu.device.clone());
                ...
                let subbuffer = {
                    ...
                    let cur_pos = camera.position.clone() + dir_light.dir * 25.0;

                    let view = Matrix4::look_at_rh(
                        cur_pos,
                        cur_pos - dir_light.dir,
